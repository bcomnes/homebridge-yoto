...
/** @import { PlatformAccessory, Characteristic, CharacteristicValue, Service, Logger } from 'homebridge' */
...
/**
 * @typedef {{ new (): Characteristic, UUID: string }} CharacteristicDefinition
 */

/**
 * @param {Service} service
 * @param {CharacteristicDefinition} characteristic
 */
function ensureCharacteristic (service, characteristic) {
  const uuid = characteristic.UUID
  const hasCharacteristic = service.characteristics.some(c => c.UUID === uuid)
  const hasOptional = service.optionalCharacteristics.some(c => c.UUID === uuid)
  if (!hasCharacteristic && !hasOptional) {
...
export class YotoTelevisionAccessory {
  ...
  setupTelevisionService () {
    ...
    ensureCharacteristic(service, Characteristic.CurrentMediaState)
    ensureCharacteristic(service, Characteristic.TargetMediaState)
  ...
  setupInputSourceService () {
    const { Service, Characteristic } = this.#platform
    const inputName = sanitizeName(this.#accessory.displayName)
  ...
  setupTelevisionSpeakerService () {
    ...
    ensureCharacteristic(service, Characteristic.Active)
    ensureCharacteristic(service, Characteristic.Volume)
    ensureCharacteristic(service, Characteristic.VolumeControlType)
    ensureCharacteristic(service, Characteristic.VolumeSelector)
  ...
  async setVolumeSelector (value) {
    ...
    const currentSteps = Number.isFinite(this.#deviceModel.status.volume)
      ? this.#deviceModel.status.volume
      : 0
    const delta = selector === Characteristic.VolumeSelector.INCREMENT ? 1 : -1
    const nextSteps = Math.max(0, Math.min(currentSteps + delta, 16))
    this.#log.debug(
      LOG_PREFIX.ACCESSORY,
