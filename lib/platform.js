...
export class YotoPlatform {
  ...
  async registerTelevisionAccessory (device, deviceModel) {
    ...
    if (this.televisionAccessories.has(uuid)) {
      this.log.debug('Television playback accessory already published:', playbackName, uuid)
      return { success: true }
    }
  ...
  removeStaleAccessories () {
    ...
    for (const [uuid, accessory] of this.televisionAccessories) {
      const deviceId = accessory.context.device?.deviceId
      if (!deviceId || !currentDeviceIds.includes(deviceId)) {
        this.log.debug('Removing external Television playback accessory from runtime:', accessory.displayName, uuid)
        ...
        this.televisionAccessories.delete(uuid)
        this.log.debug('Removed external Television playback accessory:', accessory.displayName, uuid)
      }
    }
  }
