...
export class YotoPlatform {
  ...
  constructor (log, config, api) {
    ...
    this.yotoAccount.on('error', (error, context) => {
      if (context.deviceId) {
        log.error(`Device error [${context.deviceId}]:`, error.message)
      } else {
  ...
  async registerDevice (device, deviceModel) {
    ...
    const sanitizedDeviceName = sanitizeName(device.name) || device.deviceId
    ...
    if (existingAccessory) {
      ...
      const accessory = new this.api.platformAccessory(sanitizedDeviceName, uuid, this.api.hap.Categories.SPEAKER)
