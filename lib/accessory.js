...
export class YotoPlayerAccessory {
  ...
  setupSmartSpeakerService () {
    const { Service, Characteristic } = this.#platform
    let service = this.#accessory.getService(Service.SmartSpeaker)
    if (!service) {
      service = new Service.SmartSpeaker(this.#accessory.displayName)
      this.#accessory.addService(service)
    }
    ...
    //
    // If an older version of the plugin added StatusActive, remove it (it's not a documented characteristic for SmartSpeaker).
    const existingStatusActive = service.characteristics.find(c => c.UUID === Characteristic.StatusActive.UUID)
    if (existingStatusActive) {
      service.removeCharacteristic(existingStatusActive)
    }
  ...
  setupBatteryService () {
    const { Service, Characteristic } = this.#platform
    let service = this.#accessory.getService(Service.Battery)
    if (!service) {
      service = new Service.Battery(this.generateServiceName('Battery'))
      this.#accessory.addService(service)
    }
    ...
    //   .onGet(this.getStatusActive.bind(this))
    //
    // If an older version of the plugin added StatusActive, remove it (it's not a documented characteristic for Battery).
    const existingStatusActive = service.characteristics.find(c => c.UUID === Characteristic.StatusActive.UUID)
    if (existingStatusActive) {
      service.removeCharacteristic(existingStatusActive)
    }
  ...
  setupTemperatureSensorService () {
    const { Service, Characteristic } = this.#platform
    let service = this.#accessory.getService(Service.TemperatureSensor)
    if (!service) {
      service = new Service.TemperatureSensor(this.generateServiceName('Temperature'))
      this.#accessory.addService(service)
    }
  ...
  setupNightlightServices () {
    ...
    // Day Nightlight
    let dayService = this.#accessory.getServiceById(Service.Lightbulb, 'DayNightlight')
    if (!dayService) {
      dayService = new Service.Lightbulb(this.generateServiceName('Day Nightlight'), 'DayNightlight')
      this.#accessory.addService(dayService)
    }
    ...
    // Night Nightlight
    let nightService = this.#accessory.getServiceById(Service.Lightbulb, 'NightNightlight')
    if (!nightService) {
      nightService = new Service.Lightbulb(this.generateServiceName('Night Nightlight'), 'NightNightlight')
      this.#accessory.addService(nightService)
    }
    ...
    let nightlightActiveService = this.#accessory.getServiceById(Service.ContactSensor, 'NightlightActive')
    if (!nightlightActiveService) {
      nightlightActiveService = new Service.ContactSensor(this.generateServiceName('Nightlight Active'), 'NightlightActive')
      this.#accessory.addService(nightlightActiveService)
    }
    ...
    let dayNightlightActiveService = this.#accessory.getServiceById(Service.ContactSensor, 'DayNightlightActive')
    if (!dayNightlightActiveService) {
      dayNightlightActiveService = new Service.ContactSensor(this.generateServiceName('Day Nightlight Active'), 'DayNightlightActive')
      this.#accessory.addService(dayNightlightActiveService)
    }
    ...
    let nightNightlightActiveService = this.#accessory.getServiceById(Service.ContactSensor, 'NightNightlightActive')
    if (!nightNightlightActiveService) {
      nightNightlightActiveService = new Service.ContactSensor(this.generateServiceName('Night Nightlight Active'), 'NightNightlightActive')
      this.#accessory.addService(nightNightlightActiveService)
    }
  ...
  setupCardSlotService () {
    ...
    let service = this.#accessory.getServiceById(Service.ContactSensor, 'CardSlot')
    if (!service) {
      service = new Service.ContactSensor(this.generateServiceName('Card Slot'), 'CardSlot')
      this.#accessory.addService(service)
    }
  ...
  setupNightModeService () {
    ...
    let service = this.#accessory.getServiceById(Service.OccupancySensor, 'NightModeStatus')
    if (!service) {
      service = new Service.OccupancySensor(this.generateServiceName('Night Mode'), 'NightModeStatus')
      this.#accessory.addService(service)
    }
  ...
  setupSleepTimerService () {
    ...
    let service = this.#accessory.getServiceById(Service.Switch, 'SleepTimer')
    if (!service) {
      service = new Service.Switch(this.generateServiceName('Sleep Timer'), 'SleepTimer')
      this.#accessory.addService(service)
    }
  ...
  setupBluetoothService () {
    ...
    let service = this.#accessory.getServiceById(Service.Switch, 'Bluetooth')
    if (!service) {
      service = new Service.Switch(this.generateServiceName('Bluetooth'), 'Bluetooth')
      this.#accessory.addService(service)
    }
  ...
  setupVolumeLimitServices () {
    ...
    // Day Max Volume
    let dayService = this.#accessory.getServiceById(Service.Fanv2, 'DayMaxVolume')
    if (!dayService) {
      dayService = new Service.Fanv2(this.generateServiceName('Day Max Volume'), 'DayMaxVolume')
      this.#accessory.addService(dayService)
    }
    ...
    // Night Max Volume
    let nightService = this.#accessory.getServiceById(Service.Fanv2, 'NightMaxVolume')
    if (!nightService) {
      nightService = new Service.Fanv2(this.generateServiceName('Night Max Volume'), 'NightMaxVolume')
      this.#accessory.addService(nightService)
    }
