...
export class YotoPlayerAccessory {
  ...
  /** @type {Service | undefined} */ seekBackwardService
  /** @type {Service | undefined} */ seekForwardService
  ...
  /**
   * Setup momentary seek controls as Switch services
   */
  setupSeekSwitchServices () {
    const { Service, Characteristic } = this.#platform

    // Seek backward
    const backName = this.generateServiceName('Back 30s')
    const backService = this.#accessory.getServiceById(Service.Switch, 'SeekBackward') ||
      this.#accessory.addService(Service.Switch, backName, 'SeekBackward')
    syncServiceNames({ Characteristic, service: backService, name: backName })

    backService.addOptionalCharacteristic(Characteristic.StatusActive)
    backService
      .getCharacteristic(Characteristic.StatusActive)
      .onGet(this.getStatusActive.bind(this))
    ...
    backService
      .getCharacteristic(Characteristic.On)
      .onGet(this.getMomentarySwitchOn.bind(this))
      .onSet(this.setSeekBackward.bind(this))

    this.seekBackwardService = backService
    this.#currentServices.add(backService)

    // Seek forward
    const forwardName = this.generateServiceName('Forward 30s')
    const forwardService = this.#accessory.getServiceById(Service.Switch, 'SeekForward') ||
      this.#accessory.addService(Service.Switch, forwardName, 'SeekForward')
    syncServiceNames({ Characteristic, service: forwardService, name: forwardName })

    forwardService.addOptionalCharacteristic(Characteristic.StatusActive)
    forwardService
      .getCharacteristic(Characteristic.StatusActive)
      .onGet(this.getStatusActive.bind(this))
    ...
    forwardService
      .getCharacteristic(Characteristic.On)
      .onGet(this.getMomentarySwitchOn.bind(this))
      .onSet(this.setSeekForward.bind(this))

    this.seekForwardService = forwardService
    this.#currentServices.add(forwardService)
  }
  ...
  /**
   * Momentary switches always read "off"
   * @returns {Promise<CharacteristicValue>}
   */
  async getMomentarySwitchOn () {
    return false
  }
   ...
   * @returns {Promise<void>}
   */
  async setSeekBackward (value) {
    const isOn = Boolean(value)
    if (!isOn) return

    try {
      await this.seekRelativeSeconds(-30)
    } finally {
      if (this.seekBackwardService) {
        this.seekBackwardService
  ...
  async setSeekForward (value) {
    ...
    try {
      ...
      if (this.seekForwardService) {
        this.seekForwardService
          .getCharacteristic(this.#platform.Characteristic.On)
          .updateValue(false)
      }
    }
  ...
  /**
   * Seek relative to current playback position (best-effort)
   * @param {number} deltaSeconds
   * @returns {Promise<void>}
   */
  async seekRelativeSeconds (deltaSeconds) {
    const playback = this.#deviceModel.playback
    const cardId = playback.cardId
    const position = playback.position
    const trackLength = playback.trackLength

    if (!cardId || typeof position !== 'number') {
      this.#log.warn(LOG_PREFIX.ACCESSORY, `[${this.#device.name}] Seek unavailable (no cardId/position)`)
      return
    }

    const unclamped = position + deltaSeconds
    const minSecondsIn = 0
    const maxSecondsIn = typeof trackLength === 'number' ? Math.max(0, trackLength - 1) : undefined
    const secondsIn = maxSecondsIn === undefined
      ? Math.max(minSecondsIn, unclamped)
      : Math.min(Math.max(minSecondsIn, unclamped), maxSecondsIn)

    /** @type {{ uri: string, secondsIn: number, trackKey?: string, chapterKey?: string }} */
    const command = {
      uri: `https://yoto.io/${cardId}`,
      secondsIn,
    }

    if (playback.trackKey) command.trackKey = playback.trackKey
    if (playback.chapterKey) command.chapterKey = playback.chapterKey

    this.#log.debug(LOG_PREFIX.ACCESSORY, `[${this.#device.name}] Seeking to ${secondsIn}s`)

    try {
      await this.#deviceModel.sendCommand(command)
    } catch (error) {
      this.#log.error(LOG_PREFIX.ACCESSORY, `[${this.#device.name}] Failed to seek:`, error)
      throw new this.#platform.api.hap.HapStatusError(
        this.#platform.api.hap.HAPStatus.SERVICE_COMMUNICATION_FAILURE
      )
    }
  ...
  async setVolume (value) {
    ...
    const maxVolumeSteps = this.#deviceModel.status.maxVolume || 16
    const steps = Math.max(0, Math.min(Math.round(requestedSteps), maxVolumeSteps))
    const percent = Math.round((steps / 16) * 100)
    ...
    try {
      await this.#deviceModel.sendCommand({ volume: percent })
      if (this.volumeService) {
        ...
        if (steps !== Math.round(requestedSteps)) {
          this.volumeService
            .getCharacteristic(Characteristic.RotationSpeed)
            .updateValue(steps)
        }
  ...
  updateVolumeLimitProps (maxVolume) {
    ...
    const { Characteristic } = this.#platform
    const maxVolumeSteps = maxVolume || 16
    this.volumeService
      .getCharacteristic(Characteristic.RotationSpeed)
      .setProps({
        minValue: 0,
    ...
    this.#log.debug(`[${this.#device.name}] Volume max is ${maxVolumeSteps}/16 (clamped in setter)`)
  }
  ...
  updateOnlineStatusCharacteristic (isOnline) {
    ...
    // Update playback/seek/volume services (device state)
